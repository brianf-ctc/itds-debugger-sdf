...
define(function (require) {
    ...
    MAP_REDUCE.reduce = function (context) {
        ...
        try {
            ...
            for (var fld in Current) {
                if (util.isArray(Current[fld]) && Current[fld].length == 1) {
                    Current[fld] = Current[fld][0];
                }
            }
    ...
    var Helper = {
        getParameters: function () {
            ...
            ScriptParam = {
                searchId: 'customsearch_ctc_open_po_search', //currentScript.getParameter('custscript_deb_orderstat_searchid') || ,
                vendorId: currentScript.getParameter('custscript_deb_orderstat_vendorid'),
                internalid: currentScript.getParameter('custscript_deb_orderstat_poid'),
                use_fulfill_rl: currentScript.getParameter('custscript_deb_orderstat_fulfillrl')
            };
        ...
        getLineCount: function (option) {
            ...
            try {
                ns_search.create({
                    ...
                    returnValue = parseInt(result.getValue({
                        name: 'lineuniquekey',
                        summary: ns_search.Summary.COUNT
                    }), 10) || 0;
                    return false;
