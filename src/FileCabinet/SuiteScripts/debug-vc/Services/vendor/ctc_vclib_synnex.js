 ...
 * with Catalyst Tech.
 *
 * Module Name: VC Services | Synnex API
 * @author paolodl
 * @description Vendor connector library for Synnex services and configuration.
 *
 * @NApiVersion 2.x
 * @NModuleScope Public
 *
...
define(function (require) {
    ...
    var Helper = {
        getNodeValue: function (node, xpath) {
            ...
            try {
                var xmlNode = ns_xml.XPath.select({
                    node: node,
                    xpath: xpath
                });
                if (vc2_util.isEmpty(xmlNode) || vc2_util.isEmpty(xmlNode)[0]) throw 'Empty node';

                var nodeValue = vc2_util.getNodeTextContent(xmlNode.shift());
        ...
        processItem: function (option) {
            ...
            try {
                itemObj = {
                    ...
                    line_price: vc2_util.parseFloat(
                        Helper.getNodeValue(itemNode, 'UnitPrice') || ''
                    ),
        ...
        process: function (option) {
            ...
            try {
                ...
                for (var ii = 0, jj = arrItemsNode.length; ii < jj; ii++) {
                    ...
                    vc2_util.log(logTitle, '... Duplicate Line? ', [ii, jj, dupLine]);
                    if (dupLine) continue;
                ...
                // vc2_util.dumpLog(logTitle, itemArray, 'itemArray: ');
                ...
                throw error;
                // } finally {
                //     vc2_util.dumpLog(logTitle, returnValue, '//  Response');
            }
