 ...
 * with Catalyst Tech.
 *
 * Script Name: CTC VC | Bill Creator Suitelet CS
 * Script ID: customscript_ctc_vc_cs_bc_suitelet
 * @author brianf@nscatalyst.com
 * @description Client script used by the Bill Creator Suitelet to calculate taxes and refresh totals.
 *
 * @NApiVersion 2.x
 * @NModuleScope Public
...
define(['N/ui/dialog', 'N/ui/message', 'N/currentRecord'], function (
    ...
    return {
        ...
        goToProcessBill: function () {
            var curRec = currentRecord.get();
            var url = curRec.getValue({ fieldId: 'custpage_scriptloader_url' });
            if (!url) return;

            var pw = 550, ph = 350;
            var l = Math.round(screen.width / 2 - pw / 2);
            var t = Math.round(screen.height / 2 - ph / 2);
            return window.open(
                url,
                'vcScriptLoader',
                'width=' + pw + ',height=' + ph + ',left=' + l + ',top=' + t +
