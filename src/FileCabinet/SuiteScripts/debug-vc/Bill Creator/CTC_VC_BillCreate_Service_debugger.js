            ...
            try {
                ...
                if (poData && poData.length) {
                    ...
                    if (vc2_util.isOneWorld()) {
                        BillCFG.subsidiary = currentValue.subsidiary
    ...
    var Endpoint = {
        ...
        execute: function (context) {
            ...
            try {
                ...
                var current = {
                    action: currentScript.getParameter('custscript_ctc_vcdeb_billsvc_action'),
                    poId: currentScript.getParameter('custscript_ctc_vcdeb_billsvc_poid')
                };

                var action = currentScript.action;
                if (!action) {
                    throw ns_error.create({
                        name: 'MISSING_ACTION',
                ...
                switch (action) {
                    case 'fetchbill':
                        returnValue = Actions.fetchbill(currentScript);
                        break;
                    case 'processfile':
                        returnValue = Actions.processfile(currentScript);
                        break;
                    default:
                        throw ns_error.create({
    ...
    var Actions = {
        ...
        processfile: function (option) {
            ...
            try {
                ...
                var BillCFG;
                try {
                    BillCFG = util.isString(rawBillCFG) ? JSON.parse(rawBillCFG) : rawBillCFG;
                } catch (parseErr) {
                    throw ns_error.create({
                ...
                try {
                    ...
                    throw ns_error.create({
                        ...
                        message:
                            'arrInvoices is not valid JSON: ' + vc2_util.extractError(parseErr),
                        notifyOff: true
                    });
                }
